...
r"""tgbl-comment community cluster data preparation and negative sampling.

The purpose of this binary is to prepare transfer learning sub-graphs for the
tgbl-comment dataset experiments. We sample two sub-graphs, one intended for
...
are loaded from files that result from `tgbl_comment_dataprep.py`.
...
3. tgbl_comment_node_community_map.pkl: a mapping from node ID to its community
   ID.
4. tgbl_comment_timesplit.csv: a file with the val_time and test_time columns,
   each with one entry specifying the boundaries of the val and test periods.

If any of these files are missing, run `tgbl_comment_dataprep.py` first.
...
def main(_):
  ...
  for _, row in tqdm.tqdm(tgbl_comment_edgelist.iterrows()):
    source_mapped_l.append(node_index_dict[row['source']])
    target_mapped_l.append(node_index_dict[row['target']])

  tgbl_comment_edgelist['source'] = source_mapped_l
  tgbl_comment_edgelist['target'] = target_mapped_l
  ...
  train_val_subgraph, test_subgraph = communities.get_community_cluster_nodes(
      edgelist_df=tgbl_comment_edgelist,
  ...
  train_val_edgelist = tgbl_comment_edgelist[
      tgbl_comment_edgelist['source'].isin(train_val_subgraph)
      & tgbl_comment_edgelist['target'].isin(train_val_subgraph)
  ]
  ...
  test_edgelist = tgbl_comment_edgelist[
      tgbl_comment_edgelist['source'].isin(test_subgraph)
      & tgbl_comment_edgelist['target'].isin(test_subgraph)
  ]
