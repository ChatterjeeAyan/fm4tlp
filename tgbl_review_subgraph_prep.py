...
r"""tgbl-review community cluster data preparation and negative sampling.

The purpose of this binary is to prepare transfer learning sub-graphs for the
tgbl-review dataset experiments. We sample two sub-graphs, one intended for
...
are loaded from files that result from `tgbl_review_dataprep.py`.
...
of the following files in _ROOT_DIR.value + 'datasets/tgbl_review':

1. tgbl-review_edgelist_v2.csv: the edgelist of the entire graph.
2. tgbl_review_node_index_map.pkl: a mapping from node ID to its index in the
   edgelist.
3. tgbl_review_node_community_map.pkl: a mapping from node ID to its community
   ID.
4. tgbl_review_timesplit.csv: a file with the val_time and test_time columns,
   each with one entry specifying the boundaries of the val and test periods.

If any of these files are missing, run `tgbl_review_dataprep.py` first.
...
[ General note for tgbl-review: it is a bipartite graph. The source and target
  nodes both have integer IDs, and thus might have colliding values.
  To make them unique, we make source even numbers and target odd. In particular
  the source id is multiplied by 2, and target is multiplied by 2 and then + 1.]

Example run command:

python fm4tlp/tgbl_review_subgraph_prep -- \
    --root_dir=./data
"""
...
def main(_):
  ...
  # The source and the target form a bipartite graph. The source and target
  # both have integer IDs. Their value might collide but they are different
  # nodes. To make them unique, we make source even numbers and target odd.
  tgbl_review_edgelist['source'] = tgbl_review_edgelist['source'] * 2
  tgbl_review_edgelist['target'] = tgbl_review_edgelist['target'] * 2 + 1

  with tf.io.gfile.GFile(
      os.path.join(dataset_root, 'tgbl_review_node_index_map.pkl'), 'rb'
  ) as f:
    node_index_dict = pickle.load(f)

  with tf.io.gfile.GFile(
      os.path.join(dataset_root, 'tgbl_review_node_community_map.pkl'), 'rb'
  ) as f:
    community_node_map = pickle.load(f)

  communities.reindex_communities(community_node_map, node_index_dict)

  with tf.io.gfile.GFile(dataset_root + '/tgbl_review_timesplit.csv', 'r') as f:
    timesplit = pd.read_csv(f)

  source_mapped_l = []
  target_mapped_l = []

  for _, row in tqdm.tqdm(tgbl_review_edgelist.iterrows()):
    source_mapped_l.append(node_index_dict[row['source']])
    target_mapped_l.append(node_index_dict[row['target']])

  tgbl_review_edgelist['source'] = source_mapped_l
  tgbl_review_edgelist['target'] = target_mapped_l
  ...
  train_val_subgraph, test_subgraph = communities.get_community_cluster_nodes(
      edgelist_df=tgbl_review_edgelist,
  ...
  train_val_edgelist = tgbl_review_edgelist[
      tgbl_review_edgelist['source'].isin(train_val_subgraph)
      & tgbl_review_edgelist['target'].isin(train_val_subgraph)
  ]
  ...
  with tf.io.gfile.GFile(
      os.path.join(
          dataset_root,
          'tgbl_review_' + train_val_dataset_name + '_edgelist.csv',
      ),
      'w',
  ) as f:
  ...
  filename = 'tgbl_review_' + train_val_dataset_name + '_train_edgelist.csv'
  with tf.io.gfile.GFile(os.path.join(dataset_root, filename), 'w') as f:
    community_edgelist_train.to_csv(f, index=False)

  filename = 'tgbl_review_' + train_val_dataset_name + '_val_edgelist.csv'
  with tf.io.gfile.GFile(os.path.join(dataset_root, filename), 'w') as f:
  ...
  filename = 'tgbl_review_' + train_val_dataset_name + '_val_ns.pkl'
  ...
  test_edgelist = tgbl_review_edgelist[
      tgbl_review_edgelist['source'].isin(test_subgraph)
      & tgbl_review_edgelist['target'].isin(test_subgraph)
  ]

  filename = 'tgbl_review_' + test_dataset_name + '_test_edgelist.csv'
  with tf.io.gfile.GFile(os.path.join(dataset_root, filename), 'w') as f:
  ...
  filename = 'tgbl_review_' + test_dataset_name + '_test_ns.pkl'
  with tf.io.gfile.GFile(os.path.join(dataset_root, filename), 'wb') as f:
