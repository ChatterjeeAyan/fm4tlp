...
r"""tgbl-wiki community cluster data preparation and negative sampling.

The purpose of this binary is to prepare transfer learning sub-graphs for the
tgbl-wiki dataset experiments. We sample two sub-graphs, one intended for
...
are loaded from files that result from `tgbl_wiki_dataprep.py`.
...
of the following files in _ROOT_DIR.value + 'datasets/tgbl_wiki':

1. tgbl-wiki_edgelist_v2.csv: the edgelist of the entire graph.
2. tgbl_wiki_node_index_map.pkl: a mapping from node ID to its index in the
   edgelist.
3. tgbl_wiki_node_community_map.pkl: a mapping from node ID to its community
   ID.
4. tgbl_wiki_timesplit.csv: a file with the val_time and test_time columns,
   each with one entry specifying the boundaries of the val and test periods.

If any of these files are missing, run `tgbl_wiki_dataprep.py` first.
...
[ General note for tgbl-wiki: it is a bipartite graph. The source and target
...
python fm4tlp/tgbl_wiki_subgraph_prep -- \
    --root_dir=./data
"""
...
def main(_):
  ...
  tgbl_wiki_edgelist.rename(
      columns={'user_id': 'source', 'item_id': 'target', 'timestamp': 'ts'},
      inplace=True,
  )

  with tf.io.gfile.GFile(
      os.path.join(dataset_root, 'tgbl_wiki_node_index_map.pkl'), 'rb'
  ) as f:
    node_index_dict = pickle.load(f)

  with tf.io.gfile.GFile(
      os.path.join(dataset_root, 'tgbl_wiki_node_community_map.pkl'), 'rb'
  ) as f:
    community_node_map = pickle.load(f)

  communities.reindex_communities(community_node_map, node_index_dict)

  with tf.io.gfile.GFile(dataset_root + '/tgbl_wiki_timesplit.csv', 'r') as f:
  ...
  tgbl_wiki_edgelist['source'] = source_mapped_l
  tgbl_wiki_edgelist['target'] = target_mapped_l
  ...
  train_val_subgraph, test_subgraph = communities.get_community_cluster_nodes(
      edgelist_df=tgbl_wiki_edgelist,
  ...
  train_val_edgelist = tgbl_wiki_edgelist[
      tgbl_wiki_edgelist['source'].isin(train_val_subgraph)
      & tgbl_wiki_edgelist['target'].isin(train_val_subgraph)
  ]
  ...
  with tf.io.gfile.GFile(
      os.path.join(
          dataset_root,
          'tgbl_wiki_' + train_val_dataset_name + '_edgelist.csv',
      ),
      'w',
  ) as f:
  ...
  filename = 'tgbl_wiki_' + train_val_dataset_name + '_train_edgelist.csv'
  with tf.io.gfile.GFile(os.path.join(dataset_root, filename), 'w') as f:
    community_edgelist_train.to_csv(f, index=False)

  filename = 'tgbl_wiki_' + train_val_dataset_name + '_val_edgelist.csv'
  with tf.io.gfile.GFile(os.path.join(dataset_root, filename), 'w') as f:
  ...
  filename = 'tgbl_wiki_' + train_val_dataset_name + '_val_ns.pkl'
  ...
  test_edgelist = tgbl_wiki_edgelist[
      tgbl_wiki_edgelist['source'].isin(test_subgraph)
      & tgbl_wiki_edgelist['target'].isin(test_subgraph)
  ]

  filename = 'tgbl_wiki_' + test_dataset_name + '_test_edgelist.csv'
  with tf.io.gfile.GFile(os.path.join(dataset_root, filename), 'w') as f:
  ...
  filename = 'tgbl_wiki_' + test_dataset_name + '_test_ns.pkl'
  with tf.io.gfile.GFile(os.path.join(dataset_root, filename), 'wb') as f:
