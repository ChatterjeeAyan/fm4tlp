...
_ARRAY_TASK_ID = flags.DEFINE_integer('array_task_id', 0, 'array task ID.')

_ARRAY_TASK_COUNT = flags.DEFINE_integer(
    'array_task_count', 100, 'array task count.'
)
...
def main(_):
  ...
  logging.info(
      '[!!Array] begin_index: %d end_index: %d | total: %d',
      begin_index,
      end_index,
      len(dataset_specs),
  )
  ...
  pipeline_result = beam_runner().run(
      make_pipeline(
          dataset_specs[begin_index:end_index],
          _ROOT_DIR.value,
          vac3,
          vac4,
