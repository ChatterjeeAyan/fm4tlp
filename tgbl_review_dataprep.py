...
r"""TGBL review dataprep.

Amazon product review network mapping for tgbl-review. Community detection. Time split for train, validation, and test.

Note that the source and the target form a bipartite graph. The source and target
both have integer IDs. Their value might collide but they are different nodes.
To make them unique, we make source even numbers and target odd. In particular,
the source id is multiplied by 2, and target is multiplied by 2 and then add 1.
...
def main(_):
  ...
  node_graph = nx.Graph()
  edgelist = zip(tgbl_review_edgelist.source, tgbl_review_edgelist.target)
  ...
  val_time, test_time = list(
      np.quantile(
          tgbl_review_edgelist['ts'].tolist(),
